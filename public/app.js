...
const progressPanel = (p) => `<div class="panel ${p.span ? 'span2':''}">
  <div class="panel-head"><h3>${p.title}</h3>${p.link ? `<a href="#">${p.link}</a>`:''}</div>
...
const calloutPanel = (p) => `<div class="panel ${p.span ? 'span2':''}">
  <div class="panel-head"><h3>${p.title}</h3>${p.link ? `<a href="#">${p.link}</a>`:''}</div>
...
const settingsPanel = (p) => `<div class="panel ${p.span ? 'span2':''}">
  <div class="panel-head"><h3>${p.title}</h3>${p.link ? `<a href="#">${p.link}</a>`:''}</div>
...
const TABS = [
  ...
  },
  {
    ...
    secondary: [
      { label: 'Overview', icon: I.grid },
      { label: 'Conversations', icon: I.chat, count: '92' },
      { label: 'Leads', icon: I.bolt, count: '26' },
      { label: 'Appointments', icon: I.cal, count: '17' },
      { label: 'Calls', icon: I.clock },
      { label: 'Playbooks', icon: I.doc },
    ],
  ...
  },
  {
    ...
    secondary: [
      { label: 'Overview', icon: I.grid },
      { label: 'Appointments', icon: I.cal, count: '23' },
      { label: 'Conversations', icon: I.chat },
      { label: 'Reminders', icon: I.bell, count: '12' },
      { label: 'Advisors', icon: I.user },
      { label: 'Bays', icon: I.car, count: '8' },
    ],
...
/* ---------- render ---------- */
const state = { tab: 'home', sub: {} };

function renderRail() {
  document.getElementById('railNav').innerHTML = TABS.map(t =>
    `<button class="rail-item ${t.id===state.tab?'active':''}" data-tab="${t.id}" title="${t.title}">
      ${svg(t.icon)}<span class="rl">${t.label}</span>
    </button>`).join('');
}

function renderSubmenu(tab) {
  const cur = state.sub[tab.id] ?? 0;
...
function renderCrumbs(tab) {
  ...
  document.getElementById('crumbs').innerHTML =
    `<b>${tab.title}</b><span class="sep">›</span>${tab.secondary[cur].label}`;
}

function select(tabId) {
  state.tab = tabId;
  const tab = TABS.find(t => t.id === tabId);
  renderRail(); renderSubmenu(tab); renderContent(tab); renderCrumbs(tab);
  document.getElementById('content').scrollTop = 0;
}

/* events */
document.getElementById('railNav').addEventListener('click', (e) => {
  const b = e.target.closest('.rail-item'); if (!b) return;
  select(b.dataset.tab);
});
document.getElementById('submenu').addEventListener('click', (e) => {
  const b = e.target.closest('.sub-item'); if (!b) return;
  state.sub[state.tab] = +b.dataset.sub;
  const tab = TABS.find(t => t.id === state.tab);
  renderSubmenu(tab); renderCrumbs(tab);
});
